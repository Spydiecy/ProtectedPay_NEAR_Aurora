  ...
  const connectWallet = async () => {
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        // Check if we're on the correct network (Aurora testnet)
        const network = await provider.getNetwork();
        if (network.chainId !== 1313161555) { // Aurora testnet chain ID
          try {
            await window.ethereum.request({
              method: 'wallet_switchEthereumChain',
              params: [{ chainId: '0xBA9304' }], // 1313161555 in hexadecimal
            });
          } catch (switchError) {
